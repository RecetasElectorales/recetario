...
# Modelos ####
simple_2018_fajardo_fit <- stan(file='modelos-2018/simple-2018/simple_fajardo.stan',
                    data=list(
                      N=18,
                      N_encuestadora=7,
...
simple_2018_petro_fit <- stan(file='modelos-2018/simple-2018/simple_petro.stan',
                       data=list(
                         N=18,
                         N_encuestadora=7,
...
simple_2018_duque_fit <- stan(file='modelos-2018/simple-2018/simple_duque.stan',
                     data=list(
                       N=18,
                       N_encuestadora=7,
...
simple_2018_vargaslleras_fit <- stan(file='modelos-2018/simple-2018/simple_vargaslleras.stan',
                     data=list(
                       N=18,
                       N_encuestadora=7,
...
simple_2018_delacalle_fit <- stan(file='modelos-2018/simple-2018/simple_delacalle.stan',
                     data=list(
                       N=18,
                       N_encuestadora=7,
...
simple_2018_resultados <- list()
simple_2018_resultados[["simple_2018_fajardo_fit"]] <- simple_2018_fajardo_fit %>% 
  as.matrix() %>%
  as_tibble() %>% 
...
simple_2018_resultados[["simple_2018_duque_fit"]] <- simple_2018_duque_fit %>% 
  as.matrix() %>%
  as_tibble() %>% 
  dplyr::select(starts_with("m[")) %>%
  tidyr::pivot_longer(cols = starts_with("m["), names_to = "m") 

simple_2018_resultados[["simple_2018_vargaslleras_fit"]] <- simple_2018_vargaslleras_fit %>% 
  as.matrix() %>%
  as_tibble() %>% 
  dplyr::select(starts_with("m[")) %>%
  tidyr::pivot_longer(cols = starts_with("m["), names_to = "m") 

simple_2018_resultados[["simple_2018_petro_fit"]] <- simple_2018_petro_fit %>% 
  as.matrix() %>%
  as_tibble() %>% 
  dplyr::select(starts_with("m[")) %>%
  tidyr::pivot_longer(cols = starts_with("m["), names_to = "m")
  
simple_2018_resultados[["simple_2018_delacalle_fit"]] <- simple_2018_delacalle_fit %>% 
  as.matrix() %>%
  as_tibble() %>% 
  dplyr::select(starts_with("m[")) %>%
  tidyr::pivot_longer(cols = starts_with("m["), names_to = "m") 
...
simple_2018_resultados %>%
  ...
  dplyr::left_join(tribble(~modelo,~nombre,
                           ...
                           "simple_2018_vargaslleras_fit","German Vargas Lleras"),
                   by=c("candidato"="modelo")) %>%
  readr::write_csv("modelos-2018/simple-2018/simple_2018_resultados.csv")